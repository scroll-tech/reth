...
use scroll_state_commitment::{
    test_utils::{b256_clear_msb, b256_reverse_bits, u256_clear_msb},
...
proptest! {
    ...
    #[test]
    fn fuzz_in_memory_account_nodes(mut init_state: BTreeMap<B256, (u32, U256, Option<B256>)>, state_updates: [BTreeMap<B256, Option<U256>>; 10]) {
        let init_state: BTreeMap<B256, Account> = init_state.into_iter().map(|(hashed_address, (nonce, balance, bytecode_hash))| {
            let hashed_address = b256_clear_msb(hashed_address);
        ...
        let state_updates: Vec<BTreeMap<_, _>> = state_updates.into_iter().map(|update| {
            let update = update.into_iter().map(|(hashed_address, update)| {
                let hashed_address = b256_clear_msb(hashed_address);
                let account = if let Some(balance) = update {
                    Some(Account { balance: u256_clear_msb(balance), ..Default::default() })
                } else { None };
                (hashed_address, account)
        ...
        for state_update in state_updates {
            ...
            for (hashed_address, account) in state_update.into_iter() {
            ...
            // Verify the result
            let expected_root = scroll_state_commitment::test_utils::state_root(
    ...
    fn fuzz_in_memory_storage_nodes(mut init_storage: BTreeMap<B256, U256>, storage_updates: [(bool, BTreeMap<B256, U256>); 10]) {
        ...
        let init_storage: BTreeMap<B256, U256> = init_storage.into_iter().map(|(slot, value)| {
            let hashed_slot = b256_clear_msb(slot);
        ...
        for (is_deleted, mut storage_update) in storage_updates {
            ...
            let expected_root = scroll_state_commitment::test_utils::storage_root(storage.clone());
...
fn arbitrary_storage_root() {
    proptest!(ProptestConfig::with_cases(10), |(item in arb::<(Address, std::collections::BTreeMap<B256, U256>)>())| {
        ...
        let storage: BTreeMap<B256, U256> = storage.into_iter().map(|(key, value)|  {
            let key = b256_clear_msb(key);
            tx.tx_ref().put::<tables::HashedStorages>(
                hashed_address,
                StorageEntry { key, value: value },
            )
            .unwrap();
        ...
        let expected = scroll_state_commitment::test_utils::storage_root(storage.into_iter());
