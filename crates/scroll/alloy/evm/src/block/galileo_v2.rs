...
/// L1 gas price oracle "is Galileo" slot. Added in the GalileoV2 fork.
pub const IS_GALILEO_SLOT: U256 = U256::from_limbs([12, 0, 0, 0]);
...
pub(super) fn apply_galileo_v2_hard_fork<DB: Database>(
    ...
    if state.storage(L1_GAS_PRICE_ORACLE_ADDRESS, IS_GALILEO_SLOT)? == IS_GALILEO {
        return Ok(())
    }
...
mod tests {
    ...
    fn test_apply_galileo_v2_fork() -> eyre::Result<()> {
        ...
        let oracle_storage_pre_fork = PlainStorage::from_iter([
            ...
            // penaltyThreshold
            (U256::from(9), U256::from(1_000_000_000)),
            // penaltyFactor
            (U256::from(10), U256::from(1_000_000_000)),
            // isFeynman
